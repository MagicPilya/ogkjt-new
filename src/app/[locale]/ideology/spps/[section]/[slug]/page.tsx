...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug, locale, section } = await params;
  const config = getSppsSectionConfig(section);
  if (!config || !slug || slug === "null") {
    return { title: "404" };
  }

  const result = await getArticleForLocale(config.getBySlug, slug, locale);
  if (!result) {
...
export default async function SppsSectionDetailPage({ params }: Props) {
  const { slug, locale, section } = await params;
  const config = getSppsSectionConfig(section);
  if (!config || !slug || slug === "null") notFound();
    ...
    <div className="container mx-auto px-4 py-12 max-w-4xl">
      ...
      <div className="flex justify-center mb-8">
        <Button variant="ghost" className="pl-0 hover:bg-transparent hover:text-blue-600" asChild>
          <Link href={`/${locale}/ideology/spps/${section}`}>
            <ArrowLeft className="mr-2 h-4 w-4" />
            {config.backLabel[locale]}
          </Link>
        </Button>
      </div>
