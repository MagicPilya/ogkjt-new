...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const pageTitle = pageData?.title ?? "Новости общежития";
  const title = `${pageTitle} | ${SITE_TITLE}`;
  const description = pageData?.metaDescription ?? undefined;
  return { title, description };
}

export default async function DormitoryNewsPage({ params, searchParams }: Props) {
  const { locale } = await params;
  ...
  const labels = {
    ru: { pagination: "Пагинация новостей общежития", prev: "Назад", next: "Вперед" },
    be: { pagination: "Пагінацыя навін інтэрната", prev: "Назад", next: "Наперад" },
    en: { pagination: "Dormitory news pagination", prev: "Previous", next: "Next" },
  }[locale];

  const makePageHref = (page: number) => {
    if (page <= 1) return `/${locale}/students/dormitory/news`;
    return `/${locale}/students/dormitory/news?page=${page}`;
  };
