import Link from "next/link";
import { cn } from "@/lib/utils";
import type { Locale } from "@/lib/i18n";
import { collegeNamesFallback } from "@/lib/site-defaults";

interface LogoProps {
  className?: string;
  locale?: Locale;
  line1?: string;
  line2?: string;
  fullName?: string;
  shortName?: string;
  /** Вариант для футера: крупнее и по центру */
  variant?: "default" | "footer";
}

export function Logo({
  className,
  locale = "ru",
  line1,
  line2,
  fullName,
  shortName,
  variant = "default",
}: LogoProps) {
  const isFooter = variant === "footer";
  const fallback = collegeNamesFallback[locale];
  const safeFullName = fullName || fallback.full;
  const safeShortName = shortName || fallback.short;
  const titleLine1 = line1 || fallback.main || safeFullName.split(" - ")[0] || safeFullName;
  const titleLine2 = line2 || fallback.branchShort || safeShortName;
  const homeHref = locale ? `/${locale}` : "/";

  return (
    <Link
      href={homeHref}
      className={cn(
        "flex items-center gap-2 hover:opacity-90 transition-opacity",
        isFooter && "flex-col items-center text-center gap-3",
        className
      )}
    >
      <img
        src="/icons/logo.png"
        alt={safeShortName}
        className={cn(
          "object-contain",
          isFooter ? "h-32 w-56 md:h-36 md:w-64" : "h-20 w-20"
        )}
      />
      <div className={cn("flex flex-col leading-none", isFooter && "items-center")}>
        <span className="font-bold text-lg text-slate-900 dark:text-slate-100">
          {titleLine1}
        </span>
        <span className="text-xs text-slate-500 font-medium">
          {titleLine2}
        </span>
      </div>
    </Link>
  );
}
